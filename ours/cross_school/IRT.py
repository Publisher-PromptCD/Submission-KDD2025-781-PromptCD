...
class Source_MIRTNet(nn.Module):
    ...
    def forward(self, user, item, item2):#-------------------theta 转换成 b，b 转换成 theta
        # 将参数 prompt_b 重复 n 次
        ...
        # ------------------------------
        if torch.max(new_theta != new_theta) or torch.max(new_a != new_a) or torch.max(
                new_b != new_b):  # pragma: no cover
...
class Target_MIRTNet(nn.Module):#-------------------theta 转换成 b，b 转换成 theta
...
class Target_MIRTNet2(nn.Module):#-------------------theta 转换成 b，b 转换成 theta
    def __init__(self, user_num, item_num, latent_dim, pp_dim, s_ranges, a_range=None, irf_kwargs=None):
...
class IRT():
    ...
    def Transfer_parameters(self, model, s_ranges):
        ...
        model.prompt_a.weight.data.copy_(
            self.s_irt_net.prompt_a.data)

        model.prompt_b.weight.data.copy_(
            self.s_irt_net.prompt_b.data)

        model.prompt_c.weight.data.copy_(
            self.s_irt_net.prompt_c.data)
    ...
    def draw_student_distribution(self):
        ...
        self.t_irt_net2.prompt_a.weight.data.copy_(
            self.s_irt_net.prompt_a.data)
